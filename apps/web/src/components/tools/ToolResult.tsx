...
export function ToolResult({ label, value, unit, level, explanation, large }: ToolResultProps) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-fg">{label}</p>
      <div className={flash ? 'tool-pop' : undefined}>
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          {unit && (
            <span className={`font-medium text-muted-fg ${large ? 'text-xl' : 'text-sm'}`}>
              {unit}
            </span>
          )}
        </div>
      ...
      {explanation && (
        <p
          className={`text-sm leading-relaxed ${
            large
              ? `font-medium ${level ? levelText[level] : 'text-foreground/80'}`
              : 'text-muted-fg'
          }`}
        >
          {explanation}
