    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          style={{
            background: `linear-gradient(to right, var(--primary) 0%, var(--primary) ${percent}%, var(--line) ${percent}%, var(--line) 100%)`,
          }}
        />
      </div>
      {marks && marks.length > 0 && (
        <div className="relative flex justify-between px-0.5">
          {marks.map((m) => (
            <span key={m.value} className="text-[10px] text-muted-fg">{m.label}</span>
          ))}
        </div>
      )}
    </div>
