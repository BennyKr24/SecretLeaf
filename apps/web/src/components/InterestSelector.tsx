  ...
  return (
    <div className="flex flex-col items-center gap-3">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-[13px] font-semibold
                transition-[transform,border-color,background-color,color] duration-150 select-none active:scale-[0.97]
                ${active
      ...
      {INTERESTS.some(i => isActive(i)) && (
        <p className="flex items-center gap-1 text-[11px] text-emerald-500/70 mt-1">
          <CheckCircle2 className="h-3 w-3 flex-shrink-0" strokeWidth={2} /> Inhalte werden nach deinen Interessen angepasst
        </p>
      )}
    </div>
  );
