...
const wikiLinks: { slug: string; title: string; tag: string; tagColor: string; desc: string }[] = [
  ...
  },
  {
    ...
    tagColor: "border-sky-200 bg-sky-50 text-sky-700",
  ...
  },
  {
    ...
    tag: "Diagnostik · pH",
    tagColor: "border-violet-200 bg-violet-50 text-violet-700",
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white px-6 py-10">
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mt-4 rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-cyan-50 p-6 shadow-sm">
          ...
          <h1 className="mt-1 text-4xl font-bold text-slate-900">Nährstoffmängel-Lexikon Cannabis</h1>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            <span className="rounded-full border border-emerald-200 bg-white px-3 py-1 font-semibold text-emerald-800">{deficiencyLexicon.length} Einträge</span>
            <span className="rounded-full border border-cyan-200 bg-white px-3 py-1 font-semibold text-cyan-800">Mit Diagnose-Logik</span>
        ...
        <section id="filter" className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
          <div className="grid gap-3 md:grid-cols-[1.2fr_repeat(4,minmax(0,1fr))]">
            ...
            />
            <select value={activeCategory} onChange={(e) => setActiveCategory(e.target.value as DefCategory | "alle")} className="rounded-xl border border-slate-300 px-3 py-2 text-sm">
          ...
          <p className="mt-4 text-sm text-slate-600">
            <span className="font-semibold text-slate-900">{filtered.length}</span> Treffer im Nährstoffmängel-Lexikon
          </p>
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-cyan-200 bg-cyan-50/50 p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-700">Symptome markieren und die wahrscheinlichsten Mangelbilder priorisiert anzeigen lassen.</p>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {quickDiagnosis.length > 0 ? (
              quickDiagnosis.map(({ item, score }) => (
                <article key={`diag-${item.id}`} className="rounded-xl border border-cyan-200 bg-white p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-700">{item.short}</p>
            ...
            return (
              <article key={entry.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  <p className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 font-semibold text-slate-700">Mobilität: {mobilityLabel[entry.mobility]}</p>
                  ...
                  <p className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 font-semibold text-slate-700 col-span-2">Phase: {stageLabel[entry.stage]}</p>
                </div>
                ...
                <details className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <div className="mt-3 space-y-3 text-sm text-slate-700">
                    ...
                    <div>
                      <p className="font-semibold text-slate-900">Typische Ursachen</p>
                      <ul className="mt-1 list-disc space-y-1 pl-4">
                    ...
                    <div>
                      <p className="font-semibold text-slate-900">Korrektur</p>
                      <ul className="mt-1 list-disc space-y-1 pl-4">
        ...
        {/* ── Hub statt Dublette ───────────────────────────────── */}
        <section id="lebenszyklus" className="mt-6 rounded-2xl border border-violet-200 bg-violet-50/40 p-5 shadow-sm scroll-mt-24">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-violet-700">Hub statt Parallelpflege</p>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            {lifecycleHubCards.map((card) => (
              ...
              >
                ...
                <span className="mt-4 inline-flex text-xs font-semibold text-violet-700 group-hover:underline">Zum Artikel →</span>
              </Link>
            ))}
          </div>

          <div className="mt-4 rounded-xl border border-violet-200 bg-white/80 px-4 py-3 text-sm text-slate-700">
            <strong className="text-slate-900">Warum hier nur der Hub?</strong> Lebenszyklus, Substratlogik und Antagonismen werden bereits in eigenen
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-2">
          <article id="matrix" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
            <h2 className="text-xl font-bold text-slate-900">Korrektur-Matrix</h2>
            <p className="mt-1 text-sm text-slate-600">Kompakter Leitfaden von der ersten Maßnahme bis zur Nachkontrolle.</p>
              ...
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody>
                  {correctionMatrix.map((row) => (
                    <tr key={`${row.deficiency}-${row.firstStep}`} className="border-t border-slate-100">
                      <td className="px-3 py-2 font-semibold text-slate-900">{row.deficiency}</td>
                      <td className="px-3 py-2 text-slate-700">{row.firstStep}</td>
                      <td className="px-3 py-2 text-slate-700">{row.followUp}</td>
                      <td className="px-3 py-2 text-slate-700">{row.window}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <article id="ampel" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
            ...
            <p className="mt-1 text-sm text-slate-600">Was in den ersten 24 Stunden nach Mangelverdacht passieren sollte.</p>
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-2">
          <article id="quellen" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
            ...
            <p className="mt-1 text-sm text-slate-600">Direkte Referenzen für Nährstoffdiagnostik und Management.</p>
          ...
          <article id="downloads" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
            <h2 className="text-xl font-bold text-slate-900">Protokolle herunterladen</h2>
