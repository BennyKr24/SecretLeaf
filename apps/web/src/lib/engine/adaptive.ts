...
/**
 * Load the most recent weight adjustment from Supabase.
 * Falls back to baseline weights if nothing is stored.
 */
export async function loadLatestWeights(
  supabase: SupabaseClient,
  logger: PipelineLogger,
  ...
  try {
    ...
    if (error || !data) {
      logger.debug("No stored weights found, using baseline");
      return {
        weights: { ...BASELINE_WEIGHTS },
        reason: "baseline-no-stored-weights",
        basedOnStudies: 0,
        computedAt: new Date().toISOString(),
      };
    }
    ...
    // Validate stored weights have all required keys
    const requiredKeys: Array<keyof ScoringWeights> = [
      "topicFit",
      "evidenceLevel",
      "publisherQuality",
      "freshness",
    ...
    for (const key of requiredKeys) {
      if (typeof weights[key] !== "number" || !Number.isFinite(weights[key])) {
        logger.warn(`Stored weight ${key} is invalid, falling back to baseline`);
        return {
          weights: { ...BASELINE_WEIGHTS },
          reason: "invalid-stored-weights-fallback",
          basedOnStudies: 0,
          computedAt: new Date().toISOString(),
        };
      }
    ...
    logger.info("Loaded adaptive weights", {
      basedOnStudies: data.based_on_studies,
    ...
    return {
      ...
      basedOnStudies: data.based_on_studies as number,
      computedAt: data.computed_at as string,
    };
  } catch (err) {
    logger.warn("Exception loading weights, using baseline", {
      error: err instanceof Error ? err.message : String(err),
    });
    return {
      weights: { ...BASELINE_WEIGHTS },
      reason: "load-exception-fallback",
      basedOnStudies: 0,
      computedAt: new Date().toISOString(),
    };
  }
