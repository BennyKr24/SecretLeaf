...
- [x] **Nutzer-Seite neu** (§4.4): Migration `202609010000_admin_users_view` (View `admin_users_v` = `auth.users` ⋈ `user_roles` ⋈ `subscriptions`, + `user_roles.banned`). `api/admin/users` GET filtert/sortiert/paginiert **serverseitig** über die View (B1/B2 gefixt, echtes `total`). `api/admin/users/[id]` GET (Detail-Drawer: Plan/Stripe/Grows/Logs/letzte Diagnose) + PATCH (Rolle / sperren / Pro geben+entziehen, self-guards, audit mit Feld-Diff) + DELETE. `banned` wird in `getAuthenticatedUserWithRole()` erzwungen → gesperrter Nutzer bekommt überall `null`/401. `"TEAM"` aus `UserRole` entfernt (7 Dateien; `UserPlan.team` bleibt als Stripe-Tier). `adminRoute()` unterstützt jetzt `[id]`-Params. Mega-Route 785 → **276** Z. (users-Cases raus). Verifiziert: Liste/Detail/grantPro/revoke/self-guard end-to-end.
- [ ] Wachstum-Seite (§4.3): Funnel, Kohorten, Conversion
- [x] **Neuigkeiten-Editor** (`/dashboard/admin/changelog`, §4.5): Migration `202609010001_updates` (Tabelle + RLS `select where published`, aus `updates.json` geseedet via `scripts/seed-updates.mjs`). `lib/updates.ts` liest jetzt DB-first mit JSON-Fallback (Build braucht keine DB); `updates/*`-Seiten async + `dynamicParams`. `api/admin/content/updates` GET/POST + `.../[id]` PATCH/DELETE (featured = Single-Slot, audit). **`/status` „Neuigkeiten"-Block liest jetzt aus der `updates`-Tabelle** — Eintrag im Panel erscheint sofort auf `/status` (ursprüngliches Anliegen komplett geschlossen). `changelog.json` + `generate-changelog.mjs` sind damit verwaist (bewusst noch nicht gelöscht). Editor v1 = Kernfelder; rich `sections` bleiben aus dem JSON erhalten, Editor zeigt „Detailinhalt". Nav: `changelog` live.
- [ ] Content-Seite (§4.5): Studien-Verwaltung + Knowledge-Liste (eigener Schritt)
